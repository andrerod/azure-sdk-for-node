...
*
* @param {string|array} key      The array key(s).
...
exports.tryGetValueInsensitive = function (key, haystack, defaultValue) {
  if (haystack) {
    var currentKey = _.isArray(key) ? key.pop() : key;

    for (var i in haystack) {
      if (haystack.hasOwnProperty(i) && i.toString().toLowerCase() === currentKey.toString().toLowerCase()) {
        if (_.isArray(key) && key.length > 0) {
          return exports.tryGetValueInsensitive(key, haystack[i], defaultValue);
        } else {
          return haystack[i];
        }
      }
