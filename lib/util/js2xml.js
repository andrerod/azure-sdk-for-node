...
_.extend(exports, {
  ...
  serializeDocument: function (element) {
    ...
    generatedDoc = generatedDoc.begin(element.name, { version: '1.0', encoding: 'utf-8', standalone: 'yes' });
    generatedDoc = this._writeElement(generatedDoc, element);

    return generatedDoc.doc().toString();
  },

  _writeElement: function (xmlElement, element) {
    var self = this;
    ...
    if (_.isArray(element.value)) {
      element.value.forEach(function (el) {
        xmlElement = xmlElement.ele(el.name);
        xmlElement = self._writeElement(xmlElement, el);
