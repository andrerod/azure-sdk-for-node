...
var WebSiteOperations = (function() {
   ...
   * 
   * @param {List`1} [parameters.hostNames] Required. The fully qualified
   * domain name for website. Only one hostname can be specified in the
   ...
   * are deleting
   * 
   * @param {boolean} deleteAllSlots Delete all slots for the site that you are
   * deleting
   * 
   * @param {function} callback
   * 
   * @returns {Stream} The response stream.
   */
  WebSiteOperations.prototype.delete = function(webSpaceName, webSiteName, deleteEmptyServerFarm, deleteMetrics, deleteAllSlots, callback) {
    if (callback === null || callback === undefined) {
      throw new Error('callback cannot be null.');
    }
    // Validate
    ...
    if (deleteAllSlots === null || deleteAllSlots === undefined) {
      return callback(new Error('deleteAllSlots cannot be null.'));
    }
    ...
    url2 = url2 + '&deleteAllSlots=' + encodeURIComponent(deleteAllSlots.toString());
   ...
   * 
   * @param {IDictionary`2} [parameters.appSettings] A set of name/value pairs
   * that contain application settings for a site.
   * 
   ...
   * pipeline modes.
   * 
   * @param {IDictionary`2} [parameters.metadata] Contains name/value pairs for
   * source control or other information.
   * 
