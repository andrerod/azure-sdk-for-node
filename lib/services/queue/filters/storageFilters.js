...
*/
function setRequestUrlFilter(protocol, host, port, storageAccount, usePathStyleUri) {
  return function (resource, next, callback) {
    // Normalize the path
    resource.path = _getPath(resource.path, storageAccount, usePathStyleUri);
...
function defaultHeadersFilter(apiVersion, host, port) {
  return function (resource, next, callback) {
    ...
    resource.withHeader(HeaderConstants.HOST_HEADER, host + ':' + port);
