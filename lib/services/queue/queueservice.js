...
function QueueService(storageAccountOrConnectionString, storageAccessKey, host, authenticationProvider) {
  ...
  var apiVersion = HeaderConstants.TARGET_STORAGE_VERSION;

  if (!authenticationProvider) {
    authenticationProvider = new SharedKey(
  ...
  this.pipeline = requestPipeline.create(
    ...
    authFilter(authenticationProvider),
    storageFilters.setRequestUrlFilter(storageServiceSettings._queueEndpointUri, storageServiceSettings),
    storageFilters.defaultHeadersFilter(storageServiceSettings._queueEndpointUri, apiVersion),
...
QueueService.prototype.getQueueMetadata = function (queue, optionsOrCallback, callback) {
  ...
  this.pipeline(webResource, function (err, result, response) {
    ...
    if (!err) {
      ...
      queueResult.metadata = parseMetadataHeaders(response.headers);
  ...
  this.pipeline(webResource, function (err, result, response) {
    ...
    if (!err) {
      ...
      if (result.QueueMessagesList && result.QueueMessagesList.QueueMessage) {
        ...
        queueMessageResults = messages.map(function (m) { return QueueMessageResult.parse({ body: m }); });
      }
    }
