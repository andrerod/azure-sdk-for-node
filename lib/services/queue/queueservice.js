...
function QueueService(storageAccountOrConnectionString, storageAccessKey, host, authenticationProvider) {
  ...
  this.pipeline = requestPipeline.create(
    ...
    storageFilters.setRequestUrlFilter(storageServiceSettings.protocol, storageServiceSettings.host, storageServiceSettings.port),
...
// Pipeline call should look something like this
  ...
  this.pipeline(webResource, function (err, result, response, body) {
    if (!err) {
      ...
      callback(null, servicePropertiesResult.parse(body.StorageServiceProperties));
    } else {
      callback(err);
    }
  });
