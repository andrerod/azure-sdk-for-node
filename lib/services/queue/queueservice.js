...
//var authFilters = require('./filters/authfilters');
...
function QueueService(storageAccountOrConnectionString, storageAccessKey, host, authenticationProvider) {
  ...
  this.pipeline = requestPipeline.create(
    ...
    authFilters.sharedKeyFilter(this.storageAccount, this.storageAccessKey, this.usePathStyleUri),
...
// Pipeline call should look something like this
  var options = { uri: '/', qs: {} };
  options.qs[QueryStringConstants.COMP] = 'properties';
  options.qs[QueryStringConstants.RESTYPE] = 'service';

  this.pipeline.get(options, function (err, result, response, body) {
    if (!err) {
