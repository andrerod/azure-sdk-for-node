...
//var authFilters = require('./filters/sharedKeyFilter');  // TODO: change file name here
...
function QueueService(storageAccountOrConnectionString, storageAccessKey, host, authenticationProvider) {
  ...
  /*
  // Pipeline construction should look something like this:

  this.pipeline = requestPipeline.create(
    requestPipeline.logFilter,
    webResourceToOptionsFilter,
    authFilters.sharedKeySigningFilter(this.storageAccount, this.storageAccessKey, this.usePathStyleUri),
    storageFilters.fullUrlFilter(storageSettings.protocol, storageSettings.host, storageSettings.port),
    storageFilters.defaultHeadersFilter(StorageClient.DEFAULT_API_VERSION, this.host, this.port),
    storageFilters.userAgentFilter
    );

  // Thought - create a sink that has the user agent and webResourceToOptions filters already in it,
  // and build off that. Would remove repetition across clients.
  */
}
...
/*
// Pipeline call should look something like this
var webResource = WebResource.get()
  ...
  this.pipeline(webResource, function (err, result, response, body) {
    if (!err) {
      // TODO: Parsing probably isn't quite right - add filter to do the xml->json call?
      callback(null, servicePropertiesResult.parse(response.body.StorageServiceProperties));
    } else {
      callback(err);
    }
  });
