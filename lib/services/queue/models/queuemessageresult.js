...
QueueMessageResult.parse = function (response) {
  var queueMessageResult = new QueueMessageResult();

  if (response.headers) {
    QueueMessageResult.getPropertiesFromHeaders(queueMessageResult, response.headers);
  }

  for (var property in response.body) {
    if (property === Constants.MESSAGE_TEXT_ELEMENT) {
      queueMessageResult.messagetext = new Buffer(response.body[property], 'base64').toString();
    } else {
      queueMessageResult[property.toLowerCase()] = response.body[property];
    }
  }
...
QueueMessageResult.getPropertiesFromHeaders = function (queueMessageResult, headers) {
  var setmessagePropertyFromHeaders = function (messageProperty, headerProperty) {
    if (!queueMessageResult[messageProperty] && headers[headerProperty.toLowerCase()]) {
      queueMessageResult[messageProperty] = headers[headerProperty.toLowerCase()];
    }
  };
