...
* @return {Stream}
*/
BlobService.prototype.createBlockBlob = function (container, blob, optionsOrCallback, callback) {
  validate.validateArgs('createBlockBlob', function (v) {
    v.string(container, 'container');
    v.string(blob, 'blob');
    v.containerNameIsValid(container);
    v.blobNameIsValid(container, blob);
...
/*
  var options = {
    blockCount: 0,
    sendBlock: function (buffer, length, isBlock, cb) {
      if (isBlock) {
        var blockIdPrefix = options.blockIdPrefix || self.generateBlockIdPrefix();
        var blockId = self.getBlockId(blockIdPrefix, this.blockCount);

        self.createBlobBlockFromText(blockId, container, blob, buffer, function (err) {
          cb(err, blockId);
        });
      } else {
    ...
    sendCommit: function (blocks, cb) {

    }
  };
*/
  var writeStream = new BlobWriteStream(options);

  self._createBlobFromStream
};
