...
* @param {int}                [optionsOrCallback.ParallelOperationThreadCount] The Parallel operation thread count
...
BlobService.prototype._createBlobFromStream = function(container, blob, blobType, stream, streamLength, optionsOrCallback, callback) {
  ...
  chunkStream.on('data', function(data, range) {
    ...
    if (isBlockBlobUpload) {
      ...
      operation = new BatchOperation.RestOperation(self, apiName, blockId, container, blob, data, options, function(error) {
        if(!error) {
          autoIncrement();
        }
      ...
      if (azureutil.isBufferAllZero(data)) {
        ...
        operation = new BatchOperation.RestOperation(self, apiName, container, blob, data, range.start, range.end, options, function(error) {
          if(!error) {
            autoIncrement();
          }
          allocator.releaseBuffer(data);
