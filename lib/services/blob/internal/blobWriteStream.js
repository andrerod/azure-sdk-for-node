...
function BlobWriteStream(options) {
  ...
  if (!options.sendBlock) {
    throw new Error('A sendBlock function with signature function (buffer, length, isBlock, callback) is required');
  }

  if (!options.sendCommit) {
    throw new Error('A sendCommit function with signature function (blockIds, callback)');
  }
...
BlobWriteStream.prototype._sendBlock = function (isBlockList, isLastBlock) {
  ...
  if (this.currentBufferLength > 0) {
    ...
    var callback = function (err, blockId) {
      self.currentUploads--;
    ...
    this.sendBlock(this.currentBuffer, this.currentBufferLength, isBlockList, callback);
...
BlobWriteStream.prototype._sendCommit = function () {
  ...
  this.sendCommit(self.blocks, function (err) {
    self.emit('close');
  });
};
