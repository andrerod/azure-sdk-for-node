...
*
* @param {Request}     request     The request where to build the options from.
* @param {object}      options     The request options.
* @param {function(error, requestOptions)}  callback  The callback function.
* @return {undefined}
*/
StorageServiceClient.prototype._buildRequestOptions = function (request, options, callback) {
  var self = this;

  if (!request.headers || !request.headers[HeaderConstants.CONTENT_TYPE]) {
    request = request.withHeader(HeaderConstants.CONTENT_TYPE, '');
  }

  if (!request.headers || request.headers[HeaderConstants.CONTENT_LENGTH] === undefined) {
    request.withHeader(HeaderConstants.CONTENT_LENGTH, 0);
  } else if (request.headers && request.headers[HeaderConstants.CONTENT_LENGTH] === null) {
    delete request.headers[HeaderConstants.CONTENT_LENGTH];
  }

  request = request.withHeader(HeaderConstants.STORAGE_VERSION_HEADER, this.apiVersion)
    ...
    .withHeader(HeaderConstants.ACCEPT_CHARSET_HEADER, 'UTF-8')
    .withHeader(HeaderConstants.HOST_HEADER, this.host + ':' + this.port);

  if (options) {
    if (options.timeoutIntervalInMs) {
      request = request.withQueryOption(QueryStringConstants.TIMEOUT, options.timeoutIntervalInMs);
    }

    if (options.accessConditions) {
      request = request.withHeaders(options.accessConditions,
