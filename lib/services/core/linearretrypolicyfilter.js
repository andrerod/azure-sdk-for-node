...
function LinearRetryPolicyFilter(retryCount, retryInterval) {
  ...
  function newFilter(options, next, callback) {
    ...
    function retryCallback(err, result, response, body) {
      retryData = wrappedFilter.updateRetryData(retryData, err);
      if (err &&
        !azureutil.objectIsNull(response) &&
        wrappedFilter.shouldRetry(response.statusCode, retryData)) {
        ...
        if (!azureutil.objectIsNull(err)) {
          ...
          callback(retryData.error, result, response, body);
        }
      }
    }
  ...
  newFilter.handle = wrappedFilter.handle;
  newFilter.retryCount = retryCount ? retryCount : LinearRetryPolicyFilter.DEFAULT_CLIENT_RETRY_COUNT;
  newFilter.retryInterval = retryInterval ? retryInterval : LinearRetryPolicyFilter.DEFAULT_CLIENT_RETRY_INTERVAL;
  newFilter.shouldRetry = wrappedFilter.shouldRetry;
  newFilter.updateRetryData = wrappedFilter.updateRetryData;
  return newFilter;
}
