...
var DEFAULT_PROTOCOL = Constants.HTTP + '://';
...
*/
ServiceClient.DEFAULT_PROTOCOL = DEFAULT_PROTOCOL;
...
* @return {undefined}
*/
ServiceClient.prototype._executeRequest = function (webResource, payload, resultHandler, validators, options, callback) {
  var self = this;
  var processResponseCallback = function (responseObject, next) {
    if (resultHandler) {
      ...
      if (!responseObject.error && (!validators || !validators.some(function (v) { return !v(responseObject); }))) {
        responseObject.result = resultHandler.parse(responseObject.response);
      }
    }

    var finalCallback = function (returnObject) {
      if (returnObject.result !== undefined) {
        var callbackArgs = [ returnObject.error ];
        callbackArgs = callbackArgs.concat(returnObject.result);
        callbackArgs.push(returnObject.response);
