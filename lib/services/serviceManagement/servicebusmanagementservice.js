...
var namespaceNameIsValid = require('./models/namevalidation');
...
_.extend(ServiceBusManagementService.prototype, {
  ...
  */
  createNamespace: function (namespaceName, region, callback) {
    ...
    namespaceNameIsValid(namespaceName, function (err) {
  ...
  */
  verifyNamespace: function (name, callback) {
    var that = this;
    namespaceNameIsValid(name, function (err) {
      if (err) {
        return callback(err);
      }
  ...
  _getResult: function (path, queryString, verb, requestBody, responseContentElementName, callback) {
    ...
    this.performRequest(webResource, requestBody, null, function (responseObject, next) {
      var finalCallback = function (returnObject) {
        ...
        var result = returnObject.response.body;

        if (returnObject.response.headers['content-type'] &&
            returnObject.response.headers['content-type'].indexOf('application/atom+xml') !== -1) {
          result = atomHandler.parse(returnObject.response.body, responseContentElementName);
        }
