...
<<<<<<< HEAD
/**
* Returns a list of notifications hubs.
* 
* @param {object|function}    [optionsOrCallback]                                 The request options or callback function.
* @param {int}                [optionsOrCallback.top]                             The number of notification hubs to fetch.
* @param {int}                [optionsOrCallback.skip]                            The number of notification hubs to skip.
* @param {function(error, notificationHubs, response)} callback                   The callback function.
* @return {undefined}
*/
ServiceBusService.prototype.listNotificationHubs = function (optionsOrCallback, callback) {
  ...
  var webResource = WebResource.get('/$Resources/NotificationHubs');
  if (options) {
    webResource.addOptionalQueryParam(QueryStringConstants.TOP, options.top);
    webResource.addOptionalQueryParam(QueryStringConstants.SKIP, options.skip);
  }
  ...
  var processResponseCallback = function (responseObject, next) {
    ...
    if (!responseObject.error) {
      ...
      var results = [];
      if (responseObject.response.body.feed && responseObject.response.body.feed.entry) {
        results = responseObject.response.body.feed.entry;
      } else if (responseObject.response.body.entry) {
        results = [responseObject.response.body.entry];
      }

      results.forEach(function (current) {
        var result = NotificationHubResult.parse(current);
        responseObject.result.push(result);
      });
    }
...
* @param {object|function}    [optionsOrCallback]                   The request options or callback function.
* @param {array}              [optionsOrCallback.wnsCredentials]    The WNS credentials.
* @param {array}              [optionsOrCallback.apnsCredentials]   The APNS credentials.
* @param {function(error, notificationHub, response)} callback      The callback function.
* @return {undefined}
*/
ServiceBusService.prototype.createNotificationHub = function (hubName, optionsOrCallback, callback) {
  var options = null;
  if (typeof optionsOrCallback === 'function' && !callback) {
    callback = optionsOrCallback;
  } else {
  ...
  var payload = NotificationHubResult.serialize(options);

  var webResource = WebResource.put(hubName);
  webResource.addOptionalHeader(HeaderConstants.CONTENT_TYPE, 'application/atom+xml;type=entry;charset=utf-8');
  webResource.addOptionalHeader(HeaderConstants.CONTENT_LENGTH, Buffer.byteLength(payload, 'utf8'));

  var processResponseCallback = function (responseObject, next) {
    if (!responseObject.error) {
      responseObject.result = NotificationHubResult.parse(responseObject.response.body.entry);
    }
  ...
  this.performRequest(webResource, payload, options, processResponseCallback);
};

/**
* Updates a notification hub.
* 
* @param {string}             hubName                             The notification hub name.
* @param {function(error, notificationHub, response)} callback    The callback function.
* @return {undefined}
*/
ServiceBusService.prototype.updateNotificationHub = function (hubName, optionsOrCallback, callback) {
  throw new Error('Not yet implemented');
};

/**
* Reads a notification hub.
* 
* @param {string}             hubName                             The notification hub name.
* @param {function(error, notificationHub, response)} callback    The callback function.
* @return {undefined}
*/
ServiceBusService.prototype.getNotificationHub = function (hubName, optionsOrCallback, callback) {
  throw new Error('Not yet implemented');
};
...
* @return {undefined}
*/
ServiceBusService.prototype.deleteNotificationHub = function (hubName, callback) {
  this._deleteResource(hubName, callback);
};
...
* @return {undefined}
*/
ServiceBusService.prototype._deleteResource = function (resourceName, callback) {
  validateQueueName(resourceName);
  validateCallback(callback);
