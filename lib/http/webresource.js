...
WebResource.put = function (path) {
  ...
  webResource.httpVerb = HttpVerbs.PUT;
  return webResource;
};
...
WebResource.get = function (path) {
  var webResource = new WebResource();
  webResource.path = path ? encodeSpecialCharacters(path) : null;
  webResource.httpVerb = HttpVerbs.GET;
  return webResource;
};
...
WebResource.head = function (path) {
  var webResource = new WebResource();
  webResource.path = path ? encodeSpecialCharacters(path) : null;
  webResource.httpVerb = HttpVerbs.HEAD;
  return webResource;
};
...
WebResource.del = function (path) {
  var webResource = new WebResource();
  webResource.path = path ? encodeSpecialCharacters(path) : null;
  webResource.httpVerb = HttpVerbs.DELETE;
  return webResource;
};
...
WebResource.post = function (path) {
  var webResource = new WebResource();
  webResource.path = path ? encodeSpecialCharacters(path) : null;
  webResource.httpVerb = HttpVerbs.POST;
  return webResource;
};
...
WebResource.merge = function (path) {
  var webResource = new WebResource();
  webResource.path = path ? encodeSpecialCharacters(path) : null;
  webResource.httpVerb = HttpVerbs.MERGE;
  return webResource;
};
...
/**
* Adds a request body - should be either a buffer or string.
*
* @param {buffer | string} body The body to send
*
* @return {Object} the web resource.
*/
WebResource.prototype.withBody = function (body) {
...
WebResource.prototype.pipeInput = function(inputStream, destStream) {
  if (this.httpVerb === HttpVerbs.PUT || this.httpVerb === HttpVerbs.POST || this.httpVerb === HttpVerbs.MERGE) {
